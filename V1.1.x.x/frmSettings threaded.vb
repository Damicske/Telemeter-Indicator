...
#End Region
    ...
    Public Delegate Sub AsyncMethodCaller()
    ' Private caller As New AsyncMethodCaller(AddressOf BGWthrtask_RunWorkerCompleted)
    Private BGWthr As AsyncMethodCaller
    Private Shared BgwIsBusy As Boolean = False
    'Private Shared BgwThreadId As Integer = -1
    Private Shared BGWCancelAsync As Boolean = False
...
#Region "check if a update is required or not"
        If iTimerInterval < (tbInterval.Value * 6) Then
            ...
            itmp = Nothing
            Exit Sub
        ElseIf BGWIsBusy And (iTimerInterval >= (tbInterval.Value * 6 + 24)) Then
...
#End Region
        ...
        If Not BgwIsBusy And (sBGWStatus = "0") Then
...
#End Region
                ...
                Using SendPing As New NetworkInformation.Ping
                    Try
                        ...
                        If Result.Status = NetworkInformation.IPStatus.Success Then
                            ...
                            BGWCancelAsync = False
                            BGWthr = Nothing
                            BGWthr = New AsyncMethodCaller(AddressOf BGWthrTask) 'Thread(New ThreadStart(AddressOf BGWthrTask)) With {.IsBackground = True}
                            BGWthr.BeginInvoke(AddressOf BGWthrtask_RunWorkerCompleted, "Done")
                        Else
    ...
    Private Sub frmSettings_FormClosing(sender As Object, e As FormClosingEventArgs) Handles Me.FormClosing
        If bUnload = True Then
            If BgwIsBusy Then BGWCancelAsync = True
            If btnSave.Enabled = True Then
            ...
            ClearCookies()
            If Not BETA Then
                My.Settings.DataSend = mnuSettingsDataSend.Checked
            End If
...
#Region "place on screen"
        ...
        Try
            Dim initLocation As String = My.Settings.LocationMain
            If Not String.IsNullOrEmpty(initLocation) Then
                Dim parts As String() = initLocation.Split(CChar(","))
                If parts.Length >= 2 Then
                    If CInt(parts(0)) + Size.Width > My.Computer.Screen.WorkingArea.Width Then parts(0) = "0"
                    If CInt(parts(1)) + Size.Height > My.Computer.Screen.WorkingArea.Height Then parts(1) = "0"
                    il = New Point(CInt(parts(0)), CInt(parts(1)))
                End If
            End If
        Catch ex As Exception
            Add2Log("Main::LOAD_ERR: " & ex.Message)
        Finally
            Location = il
        End Try
        il = Nothing
#End Region
    ...
    Private Sub BGWthrTask()
        BgwIsBusy = True
        'BgwThreadId = Thread.CurrentThread.ManagedThreadId()
        Dim m As Match
                ...
                If m.Success Then
                    ...
                    If tmp.IndexOf("forgotLogin") = -1 Then
                        ...
                        If sMaintenance.IndexOf("verbeteringswerken") > -1 Then
                            Dim gmdthr As Thread = New Thread(New ThreadStart(AddressOf GetMaintenanceDates))
                            With gmdthr
                                .IsBackground = True
                                .Start()
                            End With
...
#End Region
    ...
    Sub BGWthrtask_RunWorkerCompleted(ByVal ar As IAsyncResult)
    ...
    Private Sub Done()
        Dim e As ComponentModel.RunWorkerCompletedEventArgs '("",  .Message = ""}, False)
...
#End Region
        ...
        cbLanguage.Enabled = True
        If sMaintenance.Length > 5 Then
            If bMaintenance And mnuSettingsShowMyTelenet.Checked And CBool(My.Settings.ShowBalloon) Then mnuMijnTelenetMention_Click(Me, e)
...
#Region "check for an program update"
        ...
        If (My.Settings.UpdChk = 1 And _time.TotalHours > 24) Or (My.Settings.UpdChk = 2 And _time.TotalDays > 7) Or (My.Settings.UpdChk = 3 And _time.TotalDays > 30) Then
            Add2Log("DEBUG: check for prg update")
            '-update
