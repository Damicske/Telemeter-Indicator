Public Class frmPopup
    Private iCounter As Short = 0, iShowTime As Short = 0
...
#Disable Warning BC42356 ' This async method lacks 'Await' operators and so will run synchronously. Consider using the 'Await' operator to await non-blocking API calls, or 'Await Task.Run(...)' to do CPU-bound work on a background thread.
    Public Async Sub ShowTip(ByVal ShowTimeInSeconds As Short, ByVal TipTitle As String, ByVal TipText As String, ByVal TipIcon As ToolTipIcon)
#Enable Warning BC42356 ' This async method lacks 'Await' operators and so will run synchronously. Consider using the 'Await' operator to await non-blocking API calls, or 'Await Task.Run(...)' to do CPU-bound work on a background thread.
        ...
        With Me
            ...
            .ClientSize = New Size(.lblText.Width + 50, .lblText.Height)
    ...
    Private Sub Timer1_Tick(sender As Object, e As EventArgs) Handles Timer1.Tick
        ...
        If iCounter > iShowTime Then
            ...
            '-fade out
            Opacity -= 0.02
            If iCounter > iShowTime + 500 Then Close()
        End If
    End Sub
...
Public Class TaskBarPosition
    ...
    Shared Function GetTaskBarPosition(ByRef CoordinateRectangle As Rectangle, ByVal hwnd As IntPtr) As Boolean
        Try
            Dim abd As New APPBARDATA With {.hwnd = hwnd}
